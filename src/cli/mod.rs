...
pub enum SkillsAction {
    ...
    /// Manage ClawHub registry integration
    Hub {
        #[command(subcommand)]
        action: SkillsHubAction,
    },
}

#[derive(Subcommand)]
pub enum SkillsHubAction {
    /// Save a ClawHub API token for authenticated operations
    Login {
        /// ClawHub token (clh_...)
        #[arg(long)]
        token: Option<String>,
    },
    /// Show authenticated ClawHub user information
    Whoami {
    ...
    Explore {
        ...
        #[arg(long, default_value_t = 25)]
        limit: u32,
        /// Sort order (newest, downloads, rating, installs, installsAllTime, trending)
    ...
    /// Uninstall a previously installed ClawHub skill
    Uninstall {
        /// Skill slug
        slug: String,
    ...
    Update {
        ...
        slug: Option<String>,
        /// Update all locked ClawHub skills
        #[arg(long)]
        all: bool,
        /// Force update even when local modifications are detected
        #[arg(long)]
        force: bool,
    },
    /// Sync local skills to ClawHub (dry-run by default)
    Sync {
        /// Additional root(s) to scan (can be a skills dir or single skill dir)
        #[arg(long)]
        root: Vec<String>,
        /// Execute publish operations (without this, only prints plan)
        #[arg(long)]
        execute: bool,
        /// Confirm publish when executing
        #[arg(long)]
        yes: bool,
        /// Version bump strategy for changed skills
        #[arg(long, default_value = "patch")]
        bump: String,
        /// Changelog text (defaults to autogenerated message)
        #[arg(long)]
